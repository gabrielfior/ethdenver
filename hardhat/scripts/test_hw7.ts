...
import { VolcanoCoinHw5 } from "../src/types";

async function main() {
  const Factory = await ethers.getContractFactory("VolcanoCoinHw7");

  const contract = (await Factory.deploy()) as VolcanoCoinHw5;
  ...
  const paymentsFail = contract.getPayments(otherAccount.address);
  await expect(paymentsFail).to.be.revertedWith("Only payments from caller are allowed");
